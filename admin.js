...
await fetch("https://kamyczkowytrip.pl/send-passport-email.php", {
  ...
  body: JSON.stringify({
    ...
    await fetch(
    "https://kamyczkowytrip.pl/send-finding-email.php",
    {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
...
if (!emailResponse.ok || !emailResult.success) {
  throw new Error(
    emailResult.message ||
    "Kamyczek został zatwierdzony, ale nie udało się wysłać e-maila."
  );
}
    showAdminMessage(
...
const emailResponse = await fetch(
  "https://kamyczkowytrip.pl/send-rejection-email.php",
  {
    method: "POST",
    headers: {
      "Content-Type": "text/plain;charset=UTF-8"
...
if (!emailResponse.ok || !emailResult.success) {
  throw new Error(
    emailResult.message ||
    "Kamyczek został odrzucony, ale nie udało się wysłać e-maila."
  );
}
    showAdminMessage(
